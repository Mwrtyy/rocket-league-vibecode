...
function runAtRenderRate(renderHz: number, seconds: number): number {
  let ticks = 0;
  const runner = new FixedStepRunner(() => { ticks += 1; });
...
describe('FixedStepRunner', () => {
  it.each([30, 60, 120, 144])('advances identical gameplay ticks at %i Hz rendering', (renderHz) => {
    expect(runAtRenderRate(renderHz, 2)).toBe(240);
  });

  it('bounds catch-up work and reports dropped time', () => {
    let ticks = 0;
    const runner = new FixedStepRunner(() => { ticks += 1; }, FIXED_DT, 4);
